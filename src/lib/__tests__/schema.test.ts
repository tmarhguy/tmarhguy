...
describe('personNode', () => {
  ...
  it('uses author name and split given/family names', () => {
    ...
    expect(node.givenName).toBe('Tyrone');
    expect(node.familyName).toBe('Marhguy');
  });
  ...
  it('includes worksFor and alumniOf', () => {
    ...
    expect(worksFor.name).toBe('University of Pennsylvania');
...
describe('writingCollectionNode', () => {
  it('is a Blog linking the WebSite, its page, and the Person publisher', () => {
    const node = writingCollectionNode('2024-01-15');
    expect(node['@type']).toBe('Blog');
    expect(node['@id']).toBe(WRITING_ID);
    expect((node.isPartOf as Record<string, unknown>)['@id']).toBe(WEBSITE_ID);
  ...
  it('omits dateModified when not provided', () => {
    expect(writingCollectionNode().dateModified).toBeUndefined();
  });
});

describe('writingEntryNode', () => {
  it('is a BlogPosting wired to the Person, writing collection, and its WebPage', () => {
    const node = writingEntryNode(mockLog);
    const url = `${SITE_URL}/writing/${mockLog.slug}/`;
    expect(node['@type']).toBe('BlogPosting');
    expect(node['@id']).toBe(`${url}#article`);
    expect(node.headline).toBe(mockLog.title);
    expect(node.description).toBe(mockLog.description);
    expect(node.datePublished).toBe(mockLog.date);
    expect(node.dateModified).toBe(mockLog.date);
    expect((node.author as Record<string, unknown>)['@id']).toBe(PERSON_ID);
    expect((node.publisher as Record<string, unknown>)['@id']).toBe(PERSON_ID);
    expect((node.isPartOf as Record<string, unknown>)['@id']).toBe(WRITING_ID);
    expect((node.mainEntityOfPage as Record<string, unknown>)['@id']).toBe(
      `${url}#webpage`,
    );
    expect(node.about).toBe('Tomato CPU');
  });

  it('uses an ImageObject mirroring the OG image with dimensions', () => {
    const image = writingEntryNode(mockLog).image as Record<string, unknown>;
    expect(image['@type']).toBe('ImageObject');
  ...
  it('accepts a representative article image and caption', () => {
    const image = writingEntryNode(mockLog, {
      url: `${SITE_URL}/images/projects/alu-card.png`,
      width: 1117,
      height: 812,
      alt: 'A representative screenshot',
    }).image as Record<string, unknown>;

    expect(image.url).toBe(`${SITE_URL}/images/projects/alu-card.png`);
...
describe('webPageNode', () => {
  it('is a WebPage linked to the site', () => {
    const url = `${SITE_URL}/writing/test-log-entry/`;
    const node = webPageNode({ url, name: 'Test Log Entry' });
